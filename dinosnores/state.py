...
class GameState:
    ...
    # -----------------------------------------------------------------------
    herbivore_eggs: dict[HerbivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in HerbivoreType}
    )
    baby_herbivores: dict[HerbivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in HerbivoreType}
    )
    adult_herbivores: dict[HerbivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in HerbivoreType}
    )

    carnivore_eggs: dict[CarnivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in CarnivoreType}
    )
    baby_carnivores: dict[CarnivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in CarnivoreType}
    )
    adult_carnivores: dict[CarnivoreType, int] = field(
        default_factory=lambda: {t: 0 for t in CarnivoreType}
    )
    ...
    # -----------------------------------------------------------------------
    shop_items_claimed: list[bool] = field(default_factory=lambda: [False] * SHOP_TOTAL_ITEMS)
